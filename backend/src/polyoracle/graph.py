...
from polyoracle.configuration import Configuration
from polyoracle.gamma import GammaMarketClient
from polyoracle.polymarket import Polymarket
from polyoracle.state import State, InputState, OutputState, TradeDecision, Token, ResearchResult
from polyoracle.objects import Market, PolymarketEvent, Trade, Tag, Article
from polyoracle.tools import (
...
from polyoracle.utils import init_model
from polyoracle.logger import log_agent_execution
...
async def fetch_market_data(state: State) -> Dict[str, Any]:
    ...
    try:
        ...
        fallback_question = state.market_question or f"Mock Market Question for ID {market_id}"
...
async def research_agent_node(
    ...
    if isinstance(last_message, ToolMessage) and last_message.name == "deep_research":
        ...
        try:
            ...
            # Only output research_report when it's newly created to avoid duplicates
            return {
                "messages": [ai_message],
    ...
    # Only output research_report if this call actually produced new results
    return {
        "messages": [response],
        "loop_step": state.loop_step + 1,
    }
...
def route_after_reflect_on_research(state: State, *, config: Optional[RunnableConfig] = None) -> Literal["research_agent", "analysis_agent", "__end__"]:
    ...
    # Force progress if we have a research report and have spent too much time in research
    if state.research_report and state.loop_step >= 3:
        print("ROUTE_AFTER_REFLECT_ON_RESEARCH - Research report exists and loop_step >= 3, forcing progress to analysis")
        state.loop_step = 0
    ...
    if last_msg.status == "success":
        ...
        if state.loop_step >= configuration.max_loops:
            print("ROUTE_AFTER_REFLECT_ON_RESEARCH - Max loops reached, forcing progress to analysis")
            state.loop_step = 0
            return "analysis_agent"
...
def route_after_reflect_on_analysis(state: State, *, config: Optional[RunnableConfig] = None) -> Literal["analysis_agent", "trade_agent", "__end__"]:
    ...
    # Force progress if we have analysis and have spent too much time in analysis
    if state.loop_step >= 3:
        print("ROUTE_AFTER_REFLECT_ON_ANALYSIS - loop_step >= 3, forcing progress to trade_agent")
        state.loop_step = 0
        return "trade_agent"
    ...
    if last_msg.status == "success":
        ...
        if state.loop_step >= configuration.max_loops:
            print("ROUTE_AFTER_REFLECT_ON_ANALYSIS - Max loops reached, forcing progress to trade_agent")
            state.loop_step = 0
            return "trade_agent"
